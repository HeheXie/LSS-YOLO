...
__all__ = ['DyHeadBlock', 'DyHeadBlockWithDCNV3', 'Fusion', 'C2f_Faster', 'C3_Faster', 'C3_ODConv', 'C2f_ODConv', 'Partial_conv3', 'C2f_Faster_EMA', 'C3_Faster_EMA', 'C2f_DBB',
           ...
           'CSP_MutilScaleEdgeInformationEnhance', 'C2f_FFCM',"MeatIESPPF","CSP_SS2DTB","HyperComputeModule"
           ]
...
class PSA_Attention(nn.Module):
    def __init__(self, dim, num_heads=8,
        ...
        self.key_dim = int(self.head_dim * attn_ratio) #4  每个头的键的维度
        self.scale = self.key_dim ** -0.5
        nh_kd = nh_kd = self.key_dim * num_heads  #32 每个头的键的维度和头的数量的乘积
        h = dim + nh_kd * 2 #dim + 64
...
#         self.c_b = Bottleneck(self.c_ch, self.c_ch, shortcut=shortcut,g=1,k=((3, 3), (3, 3)), e=1.0)
#         self.t_b = MHSA_CGLU(self.t_ch)


#         self.conv_fuse = Conv(2*c, c)

    
#     def forward(self, x):

#         cnn_branch = self.c_b(x)
#         transformer_branch = self.t_b(x)
...
class SS2DTransformerBlock(nn.Module):
    def __init__(self, c, tcr, shortcut=True) -> None:
        super().__init__()
        self.t_ch = int(c * tcr)
        self.c_ch = c - self.t_ch
...
class CSP_SS2DTB(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, tcr=0.25, shortcut=False, g=1, e=0.5):
        ...
        self.cv1 = Conv(c1, 2 * self.c, 1, 1)
        self.cv2 = Conv((2 + n) * self.c, c2, 1)  # optional act=FReLU(c2)
        self.m = nn.ModuleList(SS2DTransformerBlock(self.c, tcr, shortcut=shortcut) for _ in range(n))
...
class IESPPF(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(f"Input to IESPPF: {x.shape}")

        # y1 = torch.mean(x, 1, keepdim=True)
        # y2 = torch.max(x, 1, keepdim=True)[0]

        y = [self.cv1(x)]

        # print(f"Output from cv1: {y[0].shape}")

        y.extend(self.m(y[-1]) for _ in range(3))
        sppf_features = torch.cat(y, 1)

        # combined = torch.cat([x, y1, y2, sppf_features], dim=1)
        # combined = torch.cat([sppf_features], dim=1)

        out = self.cv2(sppf_features)

        out = out.permute(0, 2, 3, 1)

        # print(f"Output from IESPPF (after permute back): {out.shape}")  # [B, H, W, C]
...
class MeatIESPPF(nn.Module):
    def __init__(self, c1, c2, k=5):

        super().__init__()

        self.metaformer_block =MetaFormerBlockown(dim=c1, token_mixer= IESPPF(c1=c1, c2=c2, k=k))

    def forward(self, x):
        
        x = self.metaformer_block(x)
