...
class Detect_Efficient(nn.Module):
    ...
    def __init__(self, nc=80, ch=()):  # detection layer
        ...
        c1 = max(max((16, ch[0] // 4, self.reg_max * 4)), max(ch[0], min(self.nc, 100)))  # ownnn
        # self.stem = nn.ModuleList(nn.Sequential(Conv(x, c1, 3), Conv(c1, c1, 3)) for x in ch) # ownnn
        self.stem = nn.ModuleList(nn.Sequential(DSConv(x, c1, 3), DSConv(c1, c1, 3)) for x in ch) # ownnn
        self.cv2 = nn.ModuleList(nn.Conv2d(c1, 4 * self.reg_max, 1) for x in ch)# ownnn
        self.cv3 = nn.ModuleList(nn.Conv2d(c1, self.nc, 1) for x in ch)# ownnn
